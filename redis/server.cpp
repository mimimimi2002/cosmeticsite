...
struct Timer {
    uint64_t expire_at;  // expiration timestamp
    std::string key;
...
static void entry_set_ttl(
    ...
    // Entryを更新
    ent.expire_at = expire_at;

    // 新しいtimerを追加
    g_data.min_heap.push({
        expire_at,
        key
    });
}
...
static uint32_t do_zadd(const std::vector<std::string> &cmd, uint8_t *res, uint32_t *reslen) {
  ...
  ent.zset->items.push_back({member, score});

  // unefficient sort
  // used to implement thread pool
  std::sort(
    ent.zset->items.begin(),
    ent.zset->items.end(),
    [](const ZSetItem &a, const ZSetItem &b) {
...
static uint32_t do_zrange(const std::vector<std::string> &cmd, uint8_t *res, uint32_t *reslen) {
  ...
  for (const ZSetItem &item : ent.zset->items) {
      msg += item.member;
      msg += ":";
      msg += std::to_string(item.score);
      msg += " ";
  }
...
static bool do_request(const uint8_t *req, uint32_t reqlen, uint32_t *rescode, uint8_t *res, uint32_t *reslen) {
  ...
  } else if (cmd.size() == 2 && cmd[0] == "zrange") {
...
static uint32_t next_timer_ms() {
  ...
  // calculate next timeout for idle connection
  ...
  // ttl timers
  if (!g_data.min_heap.empty() && g_data.min_heap.top().expire_at < next_us) {
    next_us = g_data.min_heap.top().expire_at;
  }
...
static void process_timers() {
  ...
  // TTL timers
  while (!g_data.min_heap.empty()) {
    Timer timer = g_data.min_heap.top();

    if (timer.expire_at > now_us) {
      break;
    }

    g_data.min_heap.pop();

    auto it = g_data.db.find(timer.key.c_str());
    ...
    // 古いtimer
    if (it->second.expire_at != timer.expire_at) { continue; }
  ...
  while (true) {
    ...
    for (size_t i = 1; i < poll_args.size(); i++) {
      ...
      if (poll_args[i].revents) {
        ...
        // update timer for connection is included here
        connection_io(conn);
